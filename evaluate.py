...
# Function that scores generated text using a reward model
def score_generated_text(reward_model, reward_tokenizer, question, generated_answer, device):
    inputs = reward_tokenizer(question, generated_answer, return_tensors='pt').to(device)
    with torch.no_grad():
...
# def save_to_run_directory(run_name, test_questions, answers):
#     run_dir = f"./runs/{run_name}"
#     # if not exists, create it
#     if not os.path.exists(run_dir):
#         os.makedirs(run_dir)
#
#     qa_pairs = [{"question": q, "answer": a} for q, a in zip(test_questions, answers)]
#
#     # save questions and answers
#     qa_path = os.path.join(run_dir, "qa_pairs.json")
#     with open(qa_path, "w") as qa_file:
#         json.dump(qa_pairs, qa_file, indent=4)
#
#     print(f"Saved questions and answers to {qa_path}")
...
def get_answer(answer_input):
    sub1 = "Assistant: "
    sub2 = "Human: "
    answer_head = answer_input.find(sub1)
    question_head = answer_input.find(sub2)
    if question_head != -1:
        answer = answer_input[answer_head+len(sub1):question_head-1]
    else:
        answer = answer_input[answer_head+len(sub1):]
    answer = answer.strip()
    return answer

def main():
    device = "cuda"
    with open("sft_test.json","r") as fp:
    ...
    for dialogue in test_data:
        question_match = re.search(r"Human: (.*?)\n", dialogue)
        if question_match:
            question = question_match.group(1).strip()
            test_questions.append(question)
        #answer
        # answer = get_answer(dialogue)
        # answers.append(answer)

    print("test_data have done")
    #model before SFT
    raw_model_name  = 'gpt2-medium'
    # model_name = 'gpt2'  # 或 gpt2-medium, gpt2-large, gpt2-xl 根据需要选择

    #Load pre-trained model and tokenizer
    generate_model = AutoModelForCausalLM.from_pretrained(raw_model_name)
    generate_model.eval()
    generate_model.to(device)
    generate_tokenizer = AutoTokenizer.from_pretrained(raw_model_name)
    #genereate an answer list
    answers = []
    print("genereate answers=========================================")
    for question in test_questions:
        answer = generate_text(generate_model, generate_tokenizer, question, device)
        answers.append(answer)
    print("reward scores========================================")
    # load reward model
    ...
    for i in range(len(test_questions)):
        question_i = test_questions[i]
        answer_i = answers[i]
        score = score_generated_text(reward_model, reward_tokenizer, question_i, answer_i, device)
        print(f"the {i}th sentence score is:", score)
        total_score += score
    ...
    # save_to_run_directory(run_name, test_questions, answers)
    ...
    #
    # state_dict = torch.load('sft_sft_experiment_202311290742_final.pt')['model_state_dict']
    ...
    # print("genereate answers=========================================")
    # for question in test_questions:
    #     answer = generate_gpt2(model, question, device)
    #     answers.append(answer)
    # print("reward scores========================================")
    # # use reward model to get the reward scores
    # reward_model_name = "OpenAssistant/reward-model-deberta-v3-large-v2"
    # reward_model = AutoModelForSequenceClassification.from_pretrained(reward_model_name)
    # reward_tokenizer = AutoTokenizer.from_pretrained(reward_model_name)
    # reward_model.to(device)
